...
@section('content')
    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-xl font-semibold text-gray-800">My Teaching Schedule</h2>
        ...
        @if(isset($groupedPeriods) && count($groupedPeriods) > 0)
            ...
            <div class="mb-6">
                <h3 class="text-lg font-medium text-gray-800 mb-3">My Classes</h3>
                <div class="flex flex-wrap gap-2">
                    @foreach($classSections as $key => $classSection)
                        <a href="{{ route('teacher.timetable.class', $classSection['timetable_id']) }}" class="px-4 py-2 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            @if($period->timetable)
                                                {{ $period->timetable->class_name }} - {{ $period->timetable->section ? $period->timetable->section->name : 'Unknown' }}
                                            @else
                                                N/A
                                            @endif
                                        </td>
                            ...
                            <div class="p-4">
                                <div class="space-y-3">
                                    @foreach($groupedPeriods[$day]->sortBy('time_from') as $period)
                                        <div class="p-3 rounded-md {{ $period->period_type == 'regular' ? 'bg-green-50 border-l-4 border-green-500' : ($period->period_type == 'break' ? 'bg-yellow-50 border-l-4 border-yellow-500' : 'bg-purple-50 border-l-4 border-purple-500') }}">
                                            <div class="flex justify-between items-start">
                                                <div>
                                                    ...
                                                    <p class="text-xs text-gray-500 mt-1">
                                                        Class: {{ $period->timetable ? $period->timetable->class_name . ' - ' . ($period->timetable->section ? $period->timetable->section->name : 'Unknown') : 'N/A' }}
                                                    </p>
                                                </div>
        ...
        @else
            <div class="bg-blue-50 border-l-4 border-blue-500 text-blue-700 p-4 rounded" role="alert">
                <p>No teaching schedule available. Please contact your school administrator.</p>
            </div>
        @endif
    </div>
