    ...
    <div class="flex-1 h-screen overflow-y-auto px-6 py-24 bg-gray-50">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-semibold mb-6 text-gray-800">
                Academics / <span class="text-l text-gray-500"> {{$class->name}}</span>
            </h1>
        ...
        <div class="bg-white rounded-xl shadow-lg w-full p-6 transition-all duration-300">
            @if($students->isEmpty())
        <p class="text-gray-500">No students found in this class & section.</p>
    @else
        <table class="w-full border border-gray-300 rounded-lg">
            <thead class="bg-gray-100">
                <tr>
                    ...
                    <th class="px-4 py-2 border-b text-left">Aadhaar Number</th>
                    <th class="px-4 py-2 border-b text-left">Parent Number</th>
            ...
            <tbody>
                @foreach($students as $i => $student)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-2 border-b">{{ $student->aadhaar_number ?? 'Not Provided' }}</td>
                    <td class="px-4 py-2 border-b">
                        @if($student->father_phone_number)
                            {{ $student->father_phone_number }}
                        @elseif($student->mother_phone_number)
                            {{ $student->mother_phone_number }}
                        @else
                            Not Provided
                        @endif
                    </td>
                        <td class="px-4 py-2 border-b">{{ $student->roll_number ?? '-' }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
