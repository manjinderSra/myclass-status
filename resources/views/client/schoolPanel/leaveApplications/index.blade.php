        ...
        <div class="bg-white rounded-xl shadow-lg w-full p-6 transition-all duration-300">
            <div class="flex justify-between items-center mb-4">
                ...
                <div class="flex space-x-2">
                    <a href="{{ route('school.leaveApplications') }}" class="px-4 py-2 rounded-lg {{ !$status ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }} transition">All</a>
                    <a href="{{ route('school.leaveApplications', ['status' => 'pending']) }}" class="px-4 py-2 rounded-lg {{ $status == 'pending' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }} transition">Pending</a>
                    <a href="{{ route('school.leaveApplications', ['status' => 'approved']) }}" class="px-4 py-2 rounded-lg {{ $status == 'approved' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }} transition">Approved</a>
                    <a href="{{ route('school.leaveApplications', ['status' => 'rejected']) }}" class="px-4 py-2 rounded-lg {{ $status == 'rejected' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }} transition">Rejected</a>
                </div>
            </div>
                        ...
                        @if($leaves->count() > 0)
                            @foreach($leaves as $leave)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <a href="{{ route('school.leaveApplications.show', $leave->id) }}" class="text-indigo-600 hover:text-indigo-900 font-medium">View</a>
                                    </td>
                                </tr>
                            @endforeach
                        @else
