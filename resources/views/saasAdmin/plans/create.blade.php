        ...
        <main class="p-6 mt-16">
            <div class="bg-white rounded-lg shadow-md p-6">
                <div class="flex justify-between items-center mb-6">
                    <h1 class="text-2xl font-semibold text-gray-800">Add New Subscription Plan</h1>
                ...
                <form action="{{ route('saasAdmin.plans.store') }}" method="POST" id="planForm">
                    @csrf
                    <!-- Debug info - hidden field to help track form submission -->
                    <input type="hidden" name="debug_info" value="form_submitted_{{ time() }}">
                        ...
                        <div>
                            ...
                            <div class="relative rounded-md shadow-sm">
                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <span class="text-gray-500 sm:text-sm"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-currency-rupee" viewBox="0 0 16 16">
  <path d="M4 3.06h2.726c1.22 0 2.12.575 2.325 1.724H4v1.051h5.051C8.855 7.001 8 7.558 6.788 7.558H4v1.317L8.437 14h2.11L6.095 8.884h.855c2.316-.018 3.465-1.476 3.688-3.049H12V4.784h-1.345c-.08-.778-.357-1.335-.793-1.732H12V2H4z"/>
</svg></span>
                                </div>
                                <input type="number" step="0.01" name="price" id="price" value="{{ old('price') }}" class="w-full pl-7 pr-12 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" placeholder="0.00" required>
                            </div>
                            @error('price')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" name="max_students" id="max_students" value="{{ old('max_students', 0) }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" min="0" required>
                            @error('max_students')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="max_teachers" id="max_teachers" value="{{ old('max_teachers', 0) }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" min="0" required>
                            @error('max_teachers')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="max_staff" id="max_staff" value="{{ old('max_staff', 0) }}" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" min="0" required>
                            @error('max_staff')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex items-center space-x-6">
                            <div class="flex items-center">
                                <input type="checkbox" name="is_popular" id="is_popular" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" {{ old('is_popular') ? 'checked' : '' }}>
                                                            ...
                                                            <div class="mt-1">
                                                                <div class="flex items-center">
                                                                    @if ($feature->value_type === 'number')
                                                                        <input type="number" name="feature_values[{{ $feature->id }}]" id="feature_value_{{ $feature->id }}" placeholder="Value" class="w-full px-2 py-1 text-sm border border-gray-300 rounded-md feature-value" value="{{ old('feature_values.' . $feature->id, '') }}">
                                                                        <span class="ml-1 text-xs text-gray-500">{{ $feature->code == 'max_file_size' ? 'MB' : '' }}</span>
                                                                    @else
                                                                        <input type="text" name="feature_values[{{ $feature->id }}]" id="feature_value_{{ $feature->id }}" placeholder="Value" class="w-full px-2 py-1 text-sm border border-gray-300 rounded-md feature-value" value="{{ old('feature_values.' . $feature->id, '') }}">
                                                                    @endif
                                                                </div>
                                                            </div>
                                                        @endif
                    ...
                    <div class="flex justify-end space-x-3">
                        ...
                        <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">Create Plan</button>
                    </div>
                </form>
                ...
                <!-- Form submission debugging script -->
                <script>
